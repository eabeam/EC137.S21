...
ggplot(essential_by_borough,aes(x=BOROUGH,y=proportion)) +
  geom_col()
```

Now, plot the **count** of approved projects by borough. 
```{r}
ggplot(essential_by_borough,aes(x=BOROUGH,y=total)) +
  geom_col()
```
...
ggplot(essential_by_category,aes(x=CATEGORY,y=total)) +
...
essential_by_category_borough <- essential %>% 
...
essential_by_category_borough %>% 
  ggplot(aes(x=BOROUGH,y=CATEGORY,fill=proportion)) + 
  geom_tile() + 
  theme_minimal() +
  scale_fill_viridis_c(option = "inferno") + 
  labs(title="Share of approved projects")
```
